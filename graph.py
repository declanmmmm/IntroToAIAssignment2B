...
def build_graph(
    xlsx_path="Datasets/Dataonly.xlsx",
    site_listing_path="Datasets/SCATSSiteListingSpreadsheet.xlsx"
):
    ...
    coords = df.groupby(
        "SCATS Number"
    )[["Location", "NB_LATITUDE", "NB_LONGITUDE"]].first()

    coords["NB_LATITUDE"] = pd.to_numeric(
        coords["NB_LATITUDE"],
        errors="coerce"
    )

    coords["NB_LONGITUDE"] = pd.to_numeric(
        coords["NB_LONGITUDE"],
        errors="coerce"
    )
    ...
    coords["roads"] = coords["Location"].apply(
        get_roads
    )

    # load SCATS site descriptions
    site_lookup = {}

    try:
        sites = pd.read_excel(site_listing_path)
        ...
        for _, row in sites.iterrows():
            site_lookup[row["SCATS Number"]] = row["Location"]

    except:
        print("Could not load SCATS site listing")
    ...
    for road, site_ids in road_groups.items():
        ...
        sorted_sites = group.sort_values(
...
if __name__ == "__main__":
    ...
    for site, neighbours in edges.items():
        ...
        print(
            f"{site} ({nodes[site]['description']}) -> "
            f"{[(n, round(d, 2)) for n, d in neighbours]}"
        )